...
export default function GameCanvas() {
  ...
  // giftModelMap: { [giftId]: model } — tính từ model.gifts[]
  const { giftModelMap } = useModels();

  const spawnShipRef = useRef(null);
  // Giữ ref mới nhất để dùng trong event listener mà không re-subscribe
  const giftModelMapRef = useRef(giftModelMap);
  useEffect(() => { giftModelMapRef.current = giftModelMap; }, [giftModelMap]);
  ...
  useEffect(() => {
    const handleGift = (data) => {
      ...
      // Tìm model có giftId này trong gifts[]
      const model = giftModelMapRef.current[String(giftId)];
      if (!model) return; // không có model nào nhận gift này → bỏ qua
  ...
  const handleSimulate = () => {
    ...
    if (model && spawnShipRef.current) {
      addNotification({ user: "Test User", giftName: "Test Gift", imgUrl: null });
      spawnShipRef.current({
        type:     model.id,
        damage:   model.damage  ?? 1,
        fireRate: model.fireRate ?? 1.0,
      ...
      spawnShipRef.current?.({
        type: "spaceship_1", damage: 1, fireRate: 1.0,
        nickname: "Test User",
        avatarUrl: "https://api.dicebear.com/7.x/avataaars/svg?seed=simulate",
      });
    }
    ...
    <div className="fixed inset-0 flex flex-col">
      ...
      <div className="flex-1 relative" style={{ marginTop: 52 }}>
        ...
        <div className="absolute bottom-5 left-5 flex flex-col-reverse gap-2 pointer-events-none z-[15] max-h-60 overflow-hidden">
          {notifications.map((n) => (
            <div
              ...
              style={{ background: "rgba(5,15,30,0.88)", border: "1px solid var(--color-border)", backdropFilter: "blur(8px)" }}
            >
              {n.imgUrl ? (
        ...
        <button
          ...
          style={{ background: "rgba(0,245,255,0.12)", border: "1px solid rgba(0,245,255,0.35)", color: "var(--color-cyan)", fontFamily: "var(--font-game)" }}
          ...
          >
            <div
              ...
              style={{
                ...
                textShadow: gameStatus === "win" ? "0 0 40px var(--color-success)" : "0 0 40px var(--color-danger)",
            ...
            <button
              ...
              style={{ fontFamily: "var(--font-game)", background: "linear-gradient(135deg, var(--color-cyan), #0088aa)" }}
